...
export default function NonResidentialLoanHero() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch("/api/submissions", {
                ...
                body: JSON.stringify({ form_slug: "non-residential-premises", data: form }),
            });
            if (res.ok) {
                await fetch('/api/contact', {
                    ...
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        formType: 'Non Residential Premises Loan Page Form - Apply for Non-Residential Loan', // or 'Home Enquiry', 'Loan Application', etc.
                        data: form,
                    }),
                });
                setSubmitted(true);
                setForm({ fullName: "", propertyType: "", phone: "", email: "", loanAmount: "", loanNeeds: "" });
            } else {
    ...
    return (
        <div className="relative min-h-screen overflow-hidden">
            ...
            <div
                ...
                style={{
                    backgroundImage: "url('/non-residential-premises-bg.avif')",
            ...
            <div
                ...
                style={{
                    background: "linear-gradient(90deg, rgba(11, 28, 48, 0.85) 0%, rgba(11, 28, 48, 0.45) 55%, rgba(11, 28, 48, 0) 100%)",
                ...
                <AnimateOnScroll direction="left" delay={0.1}>
                    <div className="w-full lg:w-[440px] rounded-2xl p-7 shadow-2xl" style={{ background: "#fff" }}>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                <label className="font-inter block text-xs font-medium mb-1.5" style={{ color: "#333" }}>Property Type <span style={{ color: "#c9922a" }}>*</span></label>
                                <select name="propertyType" value={form.propertyType} onChange={handleChange} className="font-inter w-full px-3 py-2.5 text-sm rounded-lg border outline-none transition-colors focus:border-blue-400" style={{ borderColor: "#e2e8f0", background: "#f8fafc", color: form.propertyType ? "#333" : "#a0aec0" }} required>
                            ...
                            <div>
                                <label className="font-inter block text-xs font-medium mb-1.5" style={{ color: "#333" }}>Required Loan Amount <span style={{ color: "#c9922a" }}>*</span></label>
                                <input type="text" name="loanAmount" value={form.loanAmount} onChange={handleChange} placeholder="e.g., ₹50 Lakhs" className="font-inter w-full px-3 py-2.5 text-sm rounded-lg border outline-none transition-colors focus:border-blue-400" style={{ borderColor: "#e2e8f0", background: "#f8fafc", color: "#333" }} required />
                            </div>
                            <div>
                                <label className="font-inter block text-xs font-medium mb-1.5" style={{ color: "#333" }}>Tell us about your property & requirements</label>
                                <textarea name="loanNeeds" value={form.loanNeeds} onChange={handleChange} placeholder="Describe the commercial property you wish to purchase, lease, or refinance..." rows={3} className="font-inter w-full px-3 py-2.5 text-sm rounded-lg border outline-none transition-colors focus:border-blue-400 resize-none" style={{ borderColor: "#e2e8f0", background: "#f8fafc", color: "#333" }} />
                            </div>
