...
export default function LoanAgainstRentalsHero() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch("/api/submissions", {
                ...
                body: JSON.stringify({ form_slug: "loan-against-rentals", data: form }),
            });
            if (res.ok) {
                await fetch('/api/contact', {
                    ...
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        formType: 'Loan Against Retals Page Form - Apply for Rental Loan', // or 'Home Enquiry', 'Loan Application', etc.
                        data: form,
                    }),
                });
                setSubmitted(true);
                setForm({ fullName: "", propertyType: "", phone: "", email: "", monthlyRental: "", loanNeeds: "" });
            } else {
            ...
            <div className="relative z-10 flex flex-col lg:flex-row items-center lg:items-start justify-between px-8 lg:px-16 pt-14 pb-20 gap-10 max-w-6xl mx-auto">
                <AnimateOnScroll direction="right" delay={0.1}>
                    ...
                    <div className="flex-1 max-w-md lg:pt-8">
                        ...
                        >
                            <span style={{ color: "#D4A437", fontWeight: 600 }}>Pingal Capital</span> — {hero.tagline}
                        </p>
                    </div>
                ...
                <AnimateOnScroll direction="left" delay={0.1}>
                    ...
                    >
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                <label className="font-inter block text-xs font-medium mb-1.5" style={{ color: "#333" }}>
                                    Rental Property Type <span style={{ color: "#c9922a" }}>*</span>
                                </label>
                                <select
                                    name="propertyType"
                                    ...
                                    onChange={handleChange}
                                    className="font-inter w-full px-3 py-2.5 text-sm rounded-lg border outline-none transition-colors focus:border-blue-400"
                                    style={{ borderColor: "#e2e8f0", background: "#f8fafc", color: form.propertyType ? "#333" : "#a0aec0" }}
                                    required
                                >
                            ...
                            <div>
                                <label className="font-inter block text-xs font-medium mb-1.5" style={{ color: "#333" }}>
                                    Monthly Rental Income <span style={{ color: "#c9922a" }}>*</span>
                                </label>
                                <input
                                    type="text"
                                    name="monthlyRental"
                            ...
                            <div>
                                <label className="font-inter block text-xs font-medium mb-1.5" style={{ color: "#333" }}>
                                    Tell us about your property & funding needs
                                </label>
                                <textarea
                                    name="loanNeeds"
                                    value={form.loanNeeds}
                                    onChange={handleChange}
                                    placeholder="Describe your rental property and what you plan to use the funds for..."
                                    rows={3}
